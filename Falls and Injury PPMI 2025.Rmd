...
covariates_continuous <- c('age_at_visit','disease_duration') 
...
for(var in vars){
  ...
  # make a model 
  model <- glm(eval(str2lang(var)) ~ age_at_visit + SEX+COHORT, data = df1, family = binomial) # 
  ...
  AgeP = summary(model)$coefficients['age_at_visit','Pr(>|z|)'],
...
for(var in vars){
  ...
  model = glm(eval(str2lang(var)) ~ fall_frequency+age_at_visit + SEX+COHORT, data = df2, family = binomial) 
...
xlab('Years Since Diagnosis') +ylab('')+
scale_x_continuous(limits=c(-1, 14.5))+scale_y_continuous(labels = function(x) paste0(x, "%"), limits=c(0,100))+
theme_bw()+scale_fill_grey(name = 'Fall Frequency',labels=c('Frequent Falls', 'Rare Falls'))+labs(title='Fall Frequency by Years Since Diagnosis',tag = 'A')+
...
xlab('Years Since Diagnosis') + ylab('')+
scale_x_continuous(limits=c(-1, 14.5))+scale_y_continuous(labels = function(x) paste0(x, "%"), limits=c(0,100))+
theme_bw()+labs(title='Injury Rate by Years Since Diagnosis', tag = 'B')
...
xlab('Years Since Diagnosis')+ylab('')+
scale_x_continuous(limits=c(-1, 14.5))+scale_y_continuous(labels = function(x) paste0(x, "%"), limits=c(0,100))+
theme_bw()+labs(title='Health Care Utilization by Years Since Diagnosis', tag = 'C')
...
xlab('Years Since Diagnosis') + ylab('')+scale_y_continuous(labels = function(x) paste0(x, "%"), limits=c(0,100))+
theme_bw()+labs(title='Fall-Related Injury and Healthcare Utilization by Years Since Diagnosis', tag = 'B')+
...
for(var in varlist){ # Go through each variable 
  ...
  # Now, run a regression, then get the p values for the model
    ...
    fit <- glm(eval(str2lang(var)) ~ ThreeGroups+ age_at_visit + years_since_diagnosis, data = df, family = binomial)
    ...
    fit <- glm(eval(str2lang(var)) ~ age_at_visit + years_since_diagnosis+SEX, data = df, family = binomial)
     #w = wald.test(b=coef(fit), Sigma= vcov(fit), Terms=2:3) # result = chi2, df, p value 
      #chi2=w$result$chi2[[1]]
      #Wald = w$result$chi2[[3]]
      oddsdf = exp(cbind(OR=coef(fit), confint(fit)))
    ...
    Type = 'Categorical'
      ...
      fit = lm(eval(str2lang(var)) ~  ThreeGroups+age_at_visit +SEX, data = df)
      w = wald.test(b=coef(fit), Sigma= vcov(fit), Terms=2:3) # result = chi2, df, p value 
      chi2=w$result$chi2[[1]]
      Wald = w$result$chi2[[3]]
      oddsdf = exp(cbind(OR=coef(fit), confint(fit)))
    ...
    Type='Continuous'
    }else if(var=='age_at_visit'){# Run without as a covariate
    ...
    Type = 'Continuous'
      ...
      fit <- glm(eval(str2lang(var)) ~ ThreeGroups+ age_at_visit + SEX+years_since_diagnosis, data = df, family = binomial)
    w = wald.test(b=coef(fit), Sigma= vcov(fit), Terms=2:3) # result = chi2, df, p value 
      chi2=w$result$chi2[[1]]
      Wald = w$result$chi2[[3]]
      oddsdf = exp(cbind(OR=coef(fit), confint(fit)))
    ...
    Type = 'Categorical'
      ...
      fit = lm(eval(str2lang(var)) ~ ThreeGroups+ age_at_visit+years_since_diagnosis +SEX, data = df)
    w = wald.test(b=coef(fit), Sigma= vcov(fit), Terms=2:3) # result = chi2, df, p value 
      chi2=w$result$chi2[[1]]
      Wald = w$result$chi2[[3]]
      oddsdf = exp(cbind(OR=coef(fit), confint(fit)))
...
```{r customPlots}
g.custom = NULL
namelist = c('LEDD') # ADD YOUR LIST OF VARIABLES HERE
for(i in 1:length(namelist)){
  # get values from the results dataframe
  tempdat = fullStats[fullStats$Variable==namelist[i],]
  yvar=tempdat[1,'Variable']
...
# use all values from the cross sectional dataframe 
  if(is.null(g.custom)){
    g.custom=create_chart(xSection,'ThreeGroups',yvar,lab, nr, rf)
  }else{
  g.custom=g.custom+create_chart(xSection,'ThreeGroups',yvar,lab, nr, rf)
}
}
g.custom
```
